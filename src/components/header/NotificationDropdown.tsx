...
export default function NotificationDropdown() {
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      // silent fail — dropdown tetap bisa dibuka
    } finally {
      setLoading(false);
    }
  };
  ...
  const markOneRead = async (id: number) => {
    ...
    try {
      await fetch(`${API_BASE}/notifications/read?apikey=${API_KEY}`, {
        ...
        body: JSON.stringify({ user_id: session.user?.user_id, notification_id: id }),
      });
  ...
  const handleOpen = () => {
    setIsOpen(true);
    // Refresh setiap kali dropdown dibuka
    fetchNotifications();
  };
  ...
  // ── Render: not logged in → bell tanpa badge, tanpa data ──────────────
  if (!isLoggedIn) {
    return (
            ...
            <li key={notif.id}>
              ...
              >
                ...
                <div className="relative flex-shrink-0 mt-0.5">
                  ...
                  {!notif.is_read && (
                    <span className={`absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border-2 border-white dark:border-gray-dark ${getTypeColor(notif.type)}`} />
                  )}
                </div>

                {/* Content */}
                <span className="block flex-1 min-w-0">
                  <span className={`mb-1 block text-sm leading-snug truncate
                    ${!notif.is_read
                  ...
                  <span className="flex items-center gap-2 text-[11px] text-gray-400 dark:text-gray-500 mt-1.5">
                    <span className="capitalize">{notif.category || notif.type}</span>
...
function BellIcon({ size = 20, muted = false }: { size?: number; muted?: boolean }) {
  return (
    <svg
